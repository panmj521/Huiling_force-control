...
# from hardware.aubo_C5 import RealmanRM63
...
class Identifier:
    ...
    """
    def __init__(self,arm:RealmanRM63,sensor:XjcSensor,config_path:str,):
        self.arm   = arm
        self.sensor = sensor
    ...
    def cal_R_mat(self):
        ...
        pose = identifier.arm.get_arm_pose()
    ...
    def set_position(self,x,y,z,roll,pitch,yaw):
        ...
        res = rm_inverse_kinematics_params_t(cur, pose_command, 1)
        code, q_out = self.arm.algo_handle.rm_algo_inverse_kinematics(res)
        # print(pose_command)
        # print(res)
        # print(q_out)
        ...
        if code == 0:
            print("move to:", q_out)
            self.arm.move_joint(q_out)
        elif code == 1:
            print("逆解失败")
        
        # self.arm.robot.rm_movej_p(pose_command, 20, 0, 0, 1)
        # self.arm.robot.rm_movel(pose_command, 20, 0, 0, 1)


    def identify_param_realman(self,ready_pos,cnt,sample_num = 5):
        if cnt < 15:
...
if __name__ == '__main__':
    ...
    for i in range(10):
        try:
            sensor_name = "/dev/ttyUSB" + str(i)
            # os.system('sudo chmod 777 ' + sensor_name)
            os.system('echo %s | sudo -S %s' % ("jsfb", 'sudo chmod 777 ' + sensor_name))
            sensor = XjcSensor(sensor_name, 115200)
    ...
    ready_pos = identifier.arm.get_arm_pose()
    ready_pos = [-0.467058, -0.055689, 0.293817,  3.14,      0.,       -1.572]
    ...
    print(ready_pos)
    time.sleep(1)
    identifier.identify_param_realman(ready_pos,15)
