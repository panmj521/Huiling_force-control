...
class Identifier:
    ...
    def __init__(self,arm:AuboC5,sensor:XjcSensor,config_path:str,):
        ...
        self.observed_pts = []
        self.observed_pix = []
        self.checkerboard_offset_from_tool=[0,0,0]
        self.len_data = 0
    ...
    def function(self,ready_pos,cnt,sample_num = 5):
        if cnt < 15:
        ...
        for i in range(cnt):
            ...
            # 获取最新的RGB和深度图像
            camera_color_img, camera_depth_img, camera_intrinsics = cam.get_latest_frame()
            ...
            if checkerboard_found:
                ...
                checkerboard_x = np.multiply(checkerboard_pix[0]-camera_intrinsics['cx'],checkerboard_z/camera_intrinsics['fx'])
                checkerboard_y = np.multiply(checkerboard_pix[1]-camera_intrinsics['cy'],checkerboard_z/camera_intrinsics['fy'])
                if checkerboard_z == 0:
                    continue
            ...
            print("read done")
            pose = self.arm.robot_rpc_client.getRobotInterface(self.arm.robot_name).getRobotState().getTcpPose()
            tool_position =[pose[0],pose[1],pose[2]]
            ...
            time.sleep(0.5)
        return self.measured_pts,self.observed_pts,self.observed_pix,camera_intrinsics
...
def get_rigid_transform(A, B):
    ...
    return R, t           
def get_rigid_transform_error(z_scale,camera_intrinsics):
    global measured_pts, observed_pts, observed_pix, world2camera, intrinsics

    # Apply z offset and compute new observed points using camera intrinsics
    observed_z = observed_pts[:,2:] * z_scale
    observed_x = np.multiply(observed_pix[:,[0]]-camera_intrinsics['cx'],observed_z/camera_intrinsics['fx'])
    observed_y = np.multiply(observed_pix[:,[1]]-camera_intrinsics['cy'],observed_z/camera_intrinsics['fy'])
...
if __name__ == '__main__':
    ...
    measured_pts,observed_pts,observed_pix,camera_intrinsics =identifier.function(ready_pos,45)
