...
class AdmitHybridController(BaseController):
    ...
    def load_config(self, config_path):
        ...
        for i in range(2):
            if damp_rot[i] < 0:
                damp_rot[i] = 2 * desired_xi * np.sqrt(stiff_tran[i] * mass_tran[i])
    ...
    def step(self, dt):
        ...
        wrench_err_tcp = self.state.external_wrench_tcp - self.state.desired_wrench
        self.state.arm_desired_acc[3:] = np.linalg.inv(self.M_rot) @ (wrench_err_tcp[3:] - self.D_rot @ (self.state.arm_desired_twist[3:] - self.state.desired_twist[3:])-self.K_rot @ self.state.pose_error[3:])
        ...
        # xy导纳控制 期望目标点坐标系的xy方向下的力矩为0
        # self.state.arm_desired_acc[:2] = -self.Kd @ self.state.arm_desired_twist[:2] - self.Kp @ self.state.pose_error[:2]

        self.state.arm_desired_acc[:2] = np.linalg.inv(self.M_tran) @ (wrench_err_tcp[:2] - self.D_tran @ (self.state.arm_desired_twist[:2] - self.state.desired_twist[:2])-self.K_tran @ self.state.pose_error[:2])
        ...
        if time.time() - self.laset_print_time > 1:
            print("--------AdmithybridController-------------------------------------")
            print("arm_position:",self.state.arm_position)
            print("desired_position:",self.state.desired_position)
            print("arm_orientation",R.from_quat(self.state.arm_orientation).as_euler('xyz',degrees=False))
            print("desired_orientation",R.from_quat(self.state.desired_orientation).as_euler('xyz',degrees=False))
            print("arm_position_command",self.state.arm_position_command)
            print("arm_orientation_command",R.from_quat(self.state.arm_orientation_command).as_euler('xyz',degrees=False))
