...
def capture_and_save(cam,gridboard):
    ...
    # filter points with depth >35000 or <10
    ...
    # 将 valid_ids 转换为 numpy 数组
    valid_ids = np.array(valid_ids, dtype=np.int32)

    # 确保 valid_corners 的类型为 numpy 数组
    valid_corners = np.array(valid_corners)  # corners 是一个列表的列表，所以 dtype=object
    if len(valid_corners) > 0:
        ...
        if num:
            # 绘制标定板的位置和方向
            cv2.drawFrameAxes(rgb_image, camera_matrix, dist_coeffs, rvec, tvec, 0.1)  # 0.1 是绘制坐标轴的长度（单位：米）
